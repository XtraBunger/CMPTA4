 ...
 */
    ...
    void tDisplay(TreapNode* t, int depth = 0) const {
        if (t != nullNode) {
            tDisplay(t->right, depth + 1);

            cout << string(depth, '>') << t->element << " (Priority: " << t->priority << ")" << endl;

            tDisplay(t->left, depth + 1);
        }
    }

    int countEven(TreapNode* t) {
        if (t != nullNode) {
            int sum = countEven(t->right);  // Print right subtree first
            if (t->element % 2 == 0) {
                sum++;
            }
            sum += countEven(t->left);   // Then print left subtree
            return sum;
        }
        return 0;
    }
